...
def configDevice() {
  ...
  httpCmd("/adm/set_group.cgi?group=EVENT&event_trigger=1&event1_entry=is=1|es=0,|et=3|acts=op1:0;op2:0;email:1;ftpu:1;im:0;httpn:1;httppost:0;wlled:0;smbc:0;sd:0;op3:0;op4:0;smbc_rec:0;sd_rec:0|ei=0|ea=avi,5,15,1|en=motion")
  httpCmd("/adm/set_group.cgi?group=EVENT&event_trigger=1&event2_entry=is=1|es=0,|et=6|acts=op1:0;op2:0;email:0;ftpu:0;im:0;httpn:1;httppost:0;wlled:0;smbc:0;sd:0;op3:0;op4:0;smbc_rec:0;sd_rec:0|ei=600|ea=avi,5,15,1|en=PeriodicalEvent")
  ...
  def ftpsetting = "/adm/set_group.cgi?group=FTP&ftp1=1&ftp1_server=192.168.0.4&ftp1_account=user1&ftp1_passwd=user1_ftp&ftp1_path=/sda1/" + device.displayName.replaceAll("\\s","")
  httpCmd(ftpsetting)
  def emailsetting = "/adm/set_group.cgi?group=EMAIL&smtp_server=192.168.0.4&smtp_port=25&from_addr=" + device.displayName.replaceAll("\\s","") + "@home.com&to_addr1=" + emailaddr + "&subject=video_captured"
...
def parse(description) {
  ...
  } else if (msg?.header.contains("?event=motion")) {
    log.debug "Motion Event received"
    sendEvent(name:"motion", value: "active", descriptionText: "Motion Active")
  } else if (msg?.header.contains("?event=audio")) {
    log.debug "Audio Event received"
    sendEvent(name:"motion", value: "audio", descriptionText: "Audio Active")
  }
